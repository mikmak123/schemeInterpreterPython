...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operand = rest.map(lambda oper: scheme_eval(oper, env))
        return scheme_apply(scheme_eval(first, env), operand, env)
        # END PROBLEM 5
...
def make_let_frame(bindings, env):
    ...
    formals = bindings.map(lambda b: b.first)

    vals = bindings.map(lambda b: scheme_eval(b.second.first, env))
